...
require(arules)
rule <- apriori(dat, parameter = list(supp=0.05, conf=0.7),
...
dat_death = subset(dat, Drowning_results == "死亡") #1160
dat_survived = subset(dat, Drowning_results == "獲救") #648
n <- nrow(dat_death)
set.seed(20240104) 
sindex <- sample(n,648)
dat_death = dat_death[sindex,]
newdat = rbind(dat_death, dat_survived)
...
sort.rule <- sort.rule[!redundant]
sort.rule <- as(sort.rule,"data.frame")
write.csv(sort.rule, "Survived-rules2.csv")
